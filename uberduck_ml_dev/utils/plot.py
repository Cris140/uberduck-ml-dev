...
def save_figure_to_numpy(fig):
    """Save figure to a numpy array (corrigido para Matplotlib moderno)."""
    fig.canvas.draw()
    renderer = fig.canvas.get_renderer()
    data = np.frombuffer(renderer.buffer_rgba(), dtype=np.uint8)
    data = data.reshape(fig.canvas.get_width_height()[::-1] + (4,))  # RGBA em vez de RGB
    plt.close(fig)
    return data
